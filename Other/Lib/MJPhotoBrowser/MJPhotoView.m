...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示
        ...
        NSLog(@"%@ %@",NSStringFromCGRect(_imageView.frame),NSStringFromCGSize(_imageView.image.size));
        
        NSLog(@"if---%@",_photo.url.absoluteString);
        // 不是gif，就马上开始下载
        if (![_photo.url.absoluteString hasSuffix:@"gif"]) {
            ...
            [_imageView setImageWithURL:_photo.url placeholderImage:_photo.placeholder options:SDWebImageRetryFailed|SDWebImageLowPriority completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType) {
                ...
                // 调整frame参数
                NSLog(@"completion:%@",photo.url.absoluteString);
                [photoView adjustFrame];
            }];
        }
    } else {
        NSLog(@"else---%@",_photo.url.absoluteString);
        [self photoStartLoad];
    }

    // 调整frame参数
    NSLog(@"%@",_photo.url.absoluteString);
    [self adjustFrame];
}
...
- (void)photoStartLoad
{
    NSLog(@"photoStart:%@",_photo.url.absoluteString);
    if (_photo.image) {
        ...
        [_imageView setImageWithURL:_photo.url placeholderImage:_photo.srcImageView.image options:SDWebImageRetryFailed|SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            ...
            NSLog(@"photoStart:Comp:%@",photoView.photo.url.absoluteString);
...
{
    NSLog(@"didFinish:%@",_photo.url.absoluteString);
    if (image) {
        self.scrollEnabled = YES;
        _photo.image = image;
...
- (void)adjustFrame
{
    ...
    NSLog(@"adjustF:%@",_photo.url.absoluteString);
    // 基本尺寸参数
